...
def mapa_socios(request):
    ...
    socios_json = json.dumps([
        {'id':s.id,'nombre':s.nombre,'tipo_negocio':s.tipo_negocio,
         'direccion':s.direccion,'ciudad':s.ciudad,'estado':s.estado,
         'telefono':s.telefono,'lat':s.latitud,'lng':s.longitud}
        for s in socios if s.latitud and s.longitud
    ])
    return render(request,'socios/mapa.html',{'socios_json':socios_json,'socios':socios})
...
def portal_socio(request):
    ...
    return render(request,'socios/portal.html',{
        'socio':socio,'figuras':figuras,'mis_pedidos':mis_pedidos,
    ...
    if stripe_secret:
        try:
            ...
            session = stripe.checkout.Session.create(
                ...
                line_items=[{'price_data':{'currency':'mxn',
                    'product_data':{'name':f'{figura.nombre} x{cantidad}','description':'Figuras de Fomy — PaPeExpress'},
                    'unit_amount':int(figura.precio_venta*100)},'quantity':cantidad}],
                mode='payment',
            ...
            messages.warning(request,'Error con la pasarela de pago. Pedido registrado, te contactaremos.')
            return redirect('portal_socio')
    else:
        messages.success(request,'Pedido registrado. PaPeExpress te contactará para coordinar el pago.')
        return redirect('portal_socio')
        ...
        if stripe_secret and session_id:
            try:
                ...
                if session.payment_status == 'paid':
                    pedido.estado='pagado'; pedido.stripe_payment_intent=session.payment_intent or ''
                    pedido.pagado_en=timezone.now(); pedido.save()
                    figura=pedido.figura
                    figura.cantidad_disponible=max(0,figura.cantidad_disponible-pedido.cantidad); figura.save()
                    notificar_pago_confirmado(pedido)
            except Exception as e:
                logger.error(f'Stripe verify: {e}')
        else:
            pedido.estado='pagado'; pedido.pagado_en=timezone.now(); pedido.save()
...
def pedido_cancelado(request, pk):
    ...
    messages.warning(request,'Pago cancelado. Pedido quedó pendiente.')
    return redirect('portal_socio')
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        if pedido_id:
            try:
                ...
                if pedido.estado == 'pendiente':
                    pedido.estado='pagado'; pedido.stripe_payment_intent=session.get('payment_intent','')
                    pedido.pagado_en=timezone.now(); pedido.save()
                    figura=pedido.figura
                    figura.cantidad_disponible=max(0,figura.cantidad_disponible-pedido.cantidad); figura.save()
                    notificar_pago_confirmado(pedido)
